...
use ledger_narwhal::{BatchHeader, Data, Subdag, Transmission, TransmissionID};
...
mod valid_solutions {
    ...
    fn test_solution_limits() {
        ...
        let mut chain_builder = TestChainBuilder::from_components(private_keys.to_vec(), genesis.clone());
...
fn test_forged_block_subdags() {
    ...
    let mut quorum_blocks =
        TestChainBuilder::from_components(private_keys.to_vec(), genesis.clone()).generate_blocks(3, rng);
